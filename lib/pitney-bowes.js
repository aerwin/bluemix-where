...
var TIMEOUT = 30 * 1000; //60 seconds
	...
	return {
			...
			this.getAddresses(query, function(err, result) {
				if (!err) {
					if (result && result.addresses.length > 0) {
						...
						address.retrievalTime = result.retrievalTime;
						address.searchDistance = result.searchDistance;
						address.numberOfCandidates = result.addresses.length;
						
						// Add in the English country name for client convenience
		...
		getAddresses: function(query, callback) {
			...
			restRequest.on('complete', function(result) {
				...
				if (result instanceof Error) {
					...
					if (!result.Output) {
						...
						callback(null, {
							addresses: addresses,
							retrievalTime: timeDiff,
							searchDistance: options.query.searchDistance
						});
					}
				}
			});
			...
			restRequest.on('timeout', function(ms) {
				console.error('pitney-bowes.getAddress: TIMEOUT!!!!!!!! did not return within %d ms', ms);
